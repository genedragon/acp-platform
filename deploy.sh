...
deploy_openclaw() {
  ...
  # Auto-detect public IP for SourceCidr
  SOURCE_CIDR="${SOURCE_CIDR:-}"
  if [ -z "${SOURCE_CIDR}" ]; then
    DETECTED_IP=$(curl -s --max-time 5 https://checkip.amazonaws.com 2>/dev/null || echo "")
    if [ -n "${DETECTED_IP}" ]; then
      SOURCE_CIDR="${DETECTED_IP}/32"
      log "Auto-detected public IP: ${SOURCE_CIDR}"
    else
      SOURCE_CIDR="0.0.0.0/0"
      warn "Could not detect public IP, using 0.0.0.0/0 for SourceCidr"
    fi
  fi
